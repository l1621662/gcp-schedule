 ...
 * 课表小组件（DESIGN §3.6）。
 *
 * **单条目**（2026-09-20 改版）：选择器里只有一条「城职课表」，尺寸由
...
private fun WeekBlock(model: WidgetModel, metrics: WidgetMetrics) {
    ...
    val toWeek = actionStartActivityIntent(widgetIntent(context, edu.gcp.schedule.ROUTE_WEEK))
 ...
 *
 * `route` 用 `MainActivity` 的常量（[edu.gcp.schedule.EXTRA_ROUTE] / `ROUTE_WEEK`），
 * 不在这里另立一套——两处字符串漂移会让点击静默失效。
 */
private fun widgetIntent(context: Context, route: String?): Intent =
    Intent(context, MainActivity::class.java).apply {
        flags = Intent.FLAG_ACTIVITY_SINGLE_TOP or Intent.FLAG_ACTIVITY_CLEAR_TOP
        if (route != null) putExtra(edu.gcp.schedule.EXTRA_ROUTE, route)
    }
