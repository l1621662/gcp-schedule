...
面向 **广州城市职业学院**（正方教务）的 Android 课表 App。技术栈 Kotlin + Jetpack Compose（Material 3）。
...
2. **项目根目录**：原项目作者用的是 `F:\JUWP-schedule`（历史记录）；本改版以 GitHub 仓库
   `l1621662/gcp-schedule` 为准，本地目录名随意（如 `...\develop\GCP-Schedule`），代码里不依赖它。
...
### 3.5 作息表（2026-09-23 按本校作息更新，唯一准绳）

学校作息：**每小节 40 分钟**；大节内两小节之间休息 **5 分钟**；跨大节的间隔各不相同：
1-2 → 3-4 换教室 **20 分钟**、5-6 → 7-8 本校实际 **15 分钟**；而 3-4 → 5-6 是 **140 分钟午休**、
7-8 → 9-11 是 **85 分钟晚饭**（后两者不属于「换教室」）。5 个大节共 **11 小节**
（第 5 个大节是 9/10/11 三小节）。

| 节 | 起止 | 节 | 起止 | 节 | 起止 |
|----|------|----|------|----|------|
| 1 | 08:30–09:10 | 5 | 14:00–14:40 | 9 | 18:30–19:10 |
| 2 | 09:15–09:55 | 6 | 14:45–15:25 | 10 | 19:15–19:55 |
| 3 | 10:15–10:55 | 7 | 15:40–16:20 | 11 | 20:00–20:40 |
| 4 | 11:00–11:40 | 8 | 16:25–17:05 | | |

校验方式：从 08:30 起按「40 分钟一节 + 大节内 5 分钟」逐节递推，5 个大节结束于
09:55 / 11:40 / 15:25 / 17:05 / 20:40。单测 `TimeSlotScheduleTest` 把每个跨大节间隔
（20 / 140 / 15 / 85）逐条钉死——作息被手滑改错会立刻失败，而不是等课块错位才发现。

**`TimeSlot` 存小节（11 条），`Course.startSection/endSection` 也是小节号**，两者语义一致，不做折算。
`DefaultData.defaultTimeSlots` 就是上表。作息变更走「我的 → 课表设置 → 作息表」，届时以用户值为准：
保存时写 `time_slots` 并把该课表的 `slotsCustomized` 置位，此后 `migrateTimeSlotSchema` 跳过它，
避免下一次结构性迁移把用户改过的作息悄悄改回默认值。「恢复默认作息」会清掉该标记，重新交回迁移管辖。

`DefaultData.SLOT_SCHEMA_VERSION` 当前为 **2**：v1 = 11 条小节；v2 = 7–11 节改为本校实际作息
...
|------|---------|---------|-----------------|
| 城职课表 | 4×2 | 最小 2×2，最大 440dp（≈5×5） | 见下表「尺寸档」 |
...
- 设置页给两条替代路径：① 在负一屏右上角「+」搜索里试搜「城职课表」，搜到即可添加；
...
1. **添加到桌面**：**一条**「城职课表」（名称 + 内容摘要 + 已添加徽标 + 「添加」按钮）；
...
- 包名：`edu.gcp.schedule`（已确认）；debug 构建变体加 `.debug` 后缀，与正式包可共存安装
- 应用名：**城职课表**（已确认）/ 工程名 GCP-Schedule；debug 变体显示「城职课表 Debug」
...
- **删除再写入**：先删 description 含固定标记（`城职课表同步`，并兼容改名前的 `水贝贝课表同步`）的全部事件，再整表插入；
...
- 学期：优先读课表查询页选中的 `xnm/xqm`（`3/12/16` → 第 1/2/3 学期），抓不到就按日期推候选、逐个试；
   「教务当前学期 ≠ 本地课表学期」→ 状态区提示「教务已切换学期」，跨学期不 diff，是重建基线的机会。
- **学期字符串只有一条口径**：`domain/TermFormat.normalize`。网页导入拿到的是页面标题原文
  （`2026-2027学年第一学期`），检测路径给的是规范形式（`2026-2027-1`）——2026-09-23 真机踩过：
  两边直接比较永不相等，导致每次检测都误报「教务已切换学期」。现在导入落库前归一、
  比较时两侧都归一（老基线里的原文也能对上），考试周次估算同样先归一。
...
| P5 教务 WebView | 适配广州城市职业学院（正方） | **教务 URL** |
...
3. **包名/品牌侵权观感**：应用名「城职课表」仍含学校简称，属非官方学生作品；README/设置页均已注明非学校官方。  
...
- [x] 正式包名与应用名 → `edu.gcp.schedule` / 显示名「城职课表」（2026-09-23 定）  
...
- [x] 小组件尺寸如何暴露 → **单条目自适应**（2026-09-20 反转）：原决策「三条目目录（2×2 / 2×4 / 4×4 各一条）」
  ...
  现为一条「城职课表」，内容按实测尺寸分档（`SizeMode.Exact`）
