...
└── 我的      SettingsScreen（入口列表）
      ├── 通用：主题 · 动态取色 · 触感（全局）
      ├── 课表管理 / 课表设置（学期·作息）/ 显示设置 / 教务导入 / 上课提醒（§3.7）
      ├── 成绩查询（按学期存储与展示，页内导入，见 §4.15）
      ├── 桌面小组件（尺寸预览 · 添加 · 后台及时性权限，见 §3.6）
      ├── 日历同步（提醒时长 · 一键删除已同步，见 §4.12）
      ├── 调课自动检测（默认关闭；管配置不看报告，手动检测入口在课表页导入弹层，见 §4.17）
      ├── 课表数据（JSON 导入导出）
      └── 关于
```
...
显示设置**只有一个形态**：课表页内覆盖面板（眼睛图标，或「我的 → 显示设置」
跨 Tab 触发同一弹层——在课表上方打开，改动直接看真实网格）。曾经另有
「我的」侧的独立子页（上 45% 迷你预览 + 下 55% 选项卡），其预览受挤压、
与课表页弹层内容重复，已删除；`SubpageScreen.DISPLAY_SETTINGS` 一并移除。
...
- 空态：无课 +「从教务导入」CTA；「尚未开学/未配置学期」给「去设置学期」CTA（跳课表设置）；
  加载中给进度指示，不留白屏；空态同样显示底部固定区（快捷方式网格 + 开水卡）
- **底部固定区 = 真·贴底**（2026-09-19 自「列表尾部两项」改，用户要求底部区置底）：
  快捷方式网格 + 开水卡钉在滚动区**下方**，不随课表滚动——此前它们是 `LazyColumn` 的最后两项，
  课少时悬在屏幕中段、课多时要滑到底才看得见，且空态（贴底）与有课态（跟滚）两种表现。
  三态（加载中 / 空态 / 有课态）共用同一个 `TodayBottomDock`，位置不随状态漂移。
  - 快捷方式三列图标网格在上（§3.8），开水卡在最底；新增快捷方式按序落新行、区块向上生长
  - **高度上限 = 屏高 45%**，超出时 dock 内部可滚：8 条快捷方式 + 开水卡在大字体小屏上足以吃掉
    半屏，设上限既保住课表可视区，也保证每个入口都还能够到（不设上限会被挤出屏幕且无法访问）
  - **一键开水卡恒展示**（默认开，「我的 → 胖乖生活 → 开水设置」可关）：已登录保持一键出水交互
    （解锁按钮/进度/结果原地显示）；未登录显示**未登录态**（「胖乖生活 · 未登录」+ 引导文案），
...
**我的**（入口列表，2026-09-19 重排）

- 通用：外观主题、动态取色（Material You，默认开可关）、触感反馈、显示设置、快捷方式（§3.8）
  （均全局；显示设置 2026-09-19 起从「课表」组上移——参数不再随课表分开）
- 课表（副标题「当前：课表名」）：课表管理 / 课表设置（学期+作息）/ 教务导入 /
  调课 / 上课提醒（§3.7）/ 课表数据（导出·导入·清空）—— 课表级，随当前课表（导出与清空也是当前课表口径）
- 小组件与日历：桌面小组件（§3.6）/ 日历同步（§4.12）—— 全局
- 胖乖生活：开水入口 + 开水设置（副标题标注「第三方服务 · 非学校官方功能」）
- 关于：版本与免责声明 / 开源仓库
...
**开水设置**（我的 → 胖乖生活 → 开水设置 `SubpageScreen.WATER_SETTINGS`，2026-09-19 归并）：
两项都是今日页开水卡的行为，
原先一摊在我页胖乖卡里（点击方式）、一藏在课表显示设置弹层里（卡片显示），分居两处且后者
与胖乖无关。收进同一子页后：**显示开水卡片**（默认开，关掉今日页不再展示含未登录态）+
**点击方式**（单击/双击，对今日页开水卡与开水页大按钮同时生效）。均不依赖登录态——
未登录也能关卡片显示与调点击方式；登录/退出仍在开水页完成，本页不涉及 token。
