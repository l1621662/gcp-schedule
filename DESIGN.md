...
- ~~桌面小组件（本周/今日/明日预告）~~ → **已完成（2026-09-18）**：今日三尺寸（2×2 / 2×4 / 4×4），
  含明日预告与休息提示，见 §3.6；本周网格暂不做
- 上课提醒 + 勿扰
- WebDAV / 云备份
...
### 3.6 桌面小组件（2026-09-18，P6）

入口：**我的 → 桌面小组件**（独立二级页，`SubpageScreen.WIDGET_SETTINGS`）。

#### 三个尺寸条目

桌面小组件选择器里出现**三个独立条目**（不是一条自适应），因为用户在桌面选择器里看不到
「能拖多大」，明确列出尺寸比事后解释更省事：

| 条目 | 格位 | 默认 dp | 内容 |
|------|------|---------|------|
| 水贝贝 · 2×2 | 2 宽 × 2 高 | 110×110 | 紧凑日期行 + 焦点课（正在上课 / 下一节）：课名一行 + 时刻 + 地点 |
| 水贝贝 · 4×2 | 4 宽 × 2 高 | 250×110 | 日期行 + 焦点课（横条宽版焦点卡） |
| 水贝贝 · 4×4 | 4 宽 × 4 高 | 250×250 | 日期行 + 焦点卡（含进度条）+ 完整剩余列表 + 明日预告 |

**2026-09-19 调整（用户拍板）**：条目从 2×2 / 2×4 / 4×4 改为 **2×2 / 4×2 / 4×4**——竖条 2×4
在手机桌面上信息密度低，横条更实用。2×4 形态不删：作为 4×2 被竖向拉伸后的兜底档保留
（`SizeMode.Responsive` 仍声明 110×250 档）。**所有条目顶部都有日期行**
（`9月19日 周五 · 第 2 周`；2×2 用 10sp 紧凑样式，课名让出一行）。

三条目共用同一套渲染与同一份状态推导，仅按 `LocalSize` 分档控制信息密度
（`SizeMode.Responsive` 声明 2×2 / 4×2 / 2×4 / 4×4 四档，拉伸后按实际尺寸就近取档）。
三者 `resizeMode="horizontal|vertical"`，可再拖动改大小。

#### 内容口径：与「今日」页同一套

- 焦点课、剩余列表、明日预告的取舍**完全复用** `domain/buildTodayState()`（今日页唯一状态源，
  widget 与今日页共用同一纯函数）——已结束的课不显示、同一节课不两处出现、
  明日只在今天没有待上课程时上桌，这些规则不重写
- 今日全部上完 / 今天没课 → 焦点位换成**明日预告**（「明天 · 周五 · 3 节」+ 前几节）；
  明日也无课 → 「明天没有课 · 可以放松一下」
...
1. **添加到桌面**：三种尺寸各一行（预览缩略 + 名称 + 内容摘要 + 「添加」按钮）；
   点击走 `AppWidgetManager.requestPinAppWidget`（API 26+）。桌面不支持应用内 pin 时整段换成
   「长按桌面空白处 → 小组件」的引导文案，按钮隐藏
2. **后台及时性（可选）**：`忽略电池优化`、`允许自启动` 两行，每行显示当前状态 + 「去开启」按钮；
   文案明确「不开也能用，只是刷新可能延迟几分钟」
3. **说明**：跟随系统深浅色 + 内容与今日页一致

**首次进入自动引导**：DataStore 新键 `widget_setup_seen`；首次进入自动弹一次说明弹层
（逐条解释小组件与权限作用、可跳过），确认键直接尝试钉一个 2×4。这是
...
| 尺寸 | `SizeMode.Responsive`（2×2 / 2×4 / 4×4 / 4×2 四档），渲染按 `LocalSize` 分档；三条目各占独立 widget 子类（`getGlanceIds` 按类查询互不串扰） |
...
| 条目声明 | 3 个 `GlanceAppWidgetReceiver` + 3 份 `res/xml/widget_info_*.xml`，`updatePeriodMillis="0"` |
...
- [x] 小组件尺寸如何暴露 → **三条目目录**（2×2 / 2×4 / 4×4 各一条），非单条自适应
- [x] 小组件是否要周课表迷你网格 → **否**，本轮只做「今日」；本周网格留待后续
